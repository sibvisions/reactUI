        ...
        const initWS = (baseURL:string) => {
            ...
            let reconnectInterval = new Timer(() => {
                if (!sessionExpired) {
                    ...
                    index++
                    if (index <= 5) {
                        contextState.subscriptions.emitErrorBarProperties(false, false, true, 8, translation.get("The server not reachable!") , translation.get("The server is not reachable, trying again in 5 seconds. Retry: ") + index);
                        if (index === 1) {
                            contextState.subscriptions.emitErrorBarVisible(true);
                        }
                    }
                    else {
                        contextState.subscriptions.emitErrorBarProperties(false, false, false, 8, translation.get("The server not reachable!"), translation.get("The server is not reachable"), () => {
