...
const UITable: FC<TableProps> = (baseProps) => {
    ...
    const isSmallTheme = context.appSettings.theme === "btrs";
    ...
    const isVisible = (ele:HTMLElement, container:HTMLElement, cell:any) => {
        if (ele) {
            ...
            const eleTop = cell.rowIndex * (parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8);
            ...
            if (tableRef.current) {
                ...
                if (!loadingTable || window.getComputedStyle(loadingTable).getPropertyValue("display") !== "table") {
                    ...
                    if (pageKeyPressed.current !== false) {
                        ...
                        container.scrollTo(selectedElem ? selectedElem.offsetLeft : 0, cell.rowIndex * (parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8))
                    }
                        ...
                        if (moveDirections.visTop === CellVisibility.NOT_VISIBLE) {
                            sTop = cell.rowIndex * (parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8);
                        }
                        else if (moveDirections.visTop === CellVisibility.PART_VISIBLE) {
                            sTop = container.scrollTop + (isNext ? (parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8) : -(parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8));
                        }
    
                        container.scrollTo(sLeft, sTop);
                    }
                    else {
                        container.scrollTo(container.scrollLeft, cell.rowIndex * (parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8));
                    }
                }
            }
        }, 0)
        ...
        if(wrapRef.current){
            if(onLoadCallback) {
                if (props.preferredSize) {
                    ...
                    const prefSize:Dimension = {height: providerData.length < 10 ? providerData.length * (parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8) + (props.tableHeaderVisible !== false ? 42 : 3) : 410, width: estTableWidth+4}
        ...
        <SelectedCellContext.Provider value={selectedCellId}>
            ...
            >
                <DataTable
                    ...
                    virtualRowHeight={parseInt(window.getComputedStyle(document.documentElement).getPropertyValue(isSmallTheme ? "--btrs-table-data-height" : "--btr-table-data-height")) + 8}
                    resizableColumns
