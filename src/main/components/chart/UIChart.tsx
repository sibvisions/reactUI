...
const UIChart: FC<IChart> = (baseProps) => {
                ...
                onClick: (e:any) => {
                    ...
                    if (elementArr.length) {
                        ...
                        if (foundData) {
                            ...
                            showTopBar(context.server.sendRequest(selectReq, REQUEST_KEYWORDS.SELECT_COLUMN, true), context.server.topbar);
                        }
    ...
    return (
        <span ref={chartRef} className={concatClassnames("rc-chart-wrapper", styleClassNames)} style={layoutStyle} tabIndex={getTabIndex(props.focusable, props.tabIndex)}>
            <Chart
                id={props.name}
                className="rc-chart"
                type={chartType}
