...
const UISplitPanel: FC<ISplit & IExtendableSplitPanel & IComponentConstants> = (props) => {
    ...
    const handleResize = (firstSize: Dimension, secondSize: Dimension) => {
        const firstComponent = children.find(child => child.constraints === "FIRST_COMPONENT");
        const secondComponent = children.find(child => child.constraints === "SECOND_COMPONENT");
        if (firstComponent !== undefined && secondComponent !== undefined) {
            ...
            sizeMap.set(firstComponent.id, firstSize);
            sizeMap.set(secondComponent.id, secondSize);
