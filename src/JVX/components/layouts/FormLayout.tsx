...
const FormLayout: FC<Panel> = (baseProps) => {
    ...
    const compProps = useMemo(() => {
        const componentPropsArray = new Array<BaseComponent>();
        components.forEach(component => {
           const comp = context.contentStore.flatContent.get(component.props.id);
           if(comp){
               componentPropsArray.push(comp);
           }
        });
        return componentPropsArray;
    }, [components, context.contentStore]);


    const calculateLayout = useCallback((
            preferredCompSizes: Map<string, CSSProperties>,
            contextStyle: Map<string, CSSProperties>,
        ...
        const calculateTargetDependentAnchors = () => {
            ...
            if(calculatedTargetDependentAnchors && lba && rba && bba && tba && initSize){
            ...
            if(borderConstraint && marginConstraint){
                ...
                setCalculatedStyle( {
                    style: {
                        ...
                        left:  contextStyle.get(id)?.left || marginConstraint.leftAnchor.getAbsolutePosition(),
                        top:  contextStyle.get(id)?.top || marginConstraint.topAnchor.getAbsolutePosition(),
                        position: contextStyle.get(id)?.position || "relative",
                    },
    ...
    },[preferredComponentSizes, calculateLayout, props, compProps]);

    useLayoutEffect(() => {
        const size = dictatedStyle.get(props.id);
        if(size && preferredComponentSizes){
            initSizeRef.current = {width: (size.width as number), height: (size.height as number)}
            calculateLayout(
                preferredComponentSizes,
                dictatedStyle,
                compProps,
